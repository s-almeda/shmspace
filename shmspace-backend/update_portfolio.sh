...
# Upload new video files directly to server (bypassing git), skip existing
if [ -n "$LARGE_FILES" ]; then
    echo "📦 Checking for new video files to upload..."
    while IFS= read -r file; do
        remote_dir="$SERVER_BASE/$(dirname "$file")"
        ssh -n "$SERVER" "mkdir -p '$remote_dir'"
        rsync --ignore-existing -e ssh "$file" "$SERVER:$SERVER_BASE/$file" && echo "  ✓ uploaded $file" || true
    done <<< "$LARGE_FILES"
fi
